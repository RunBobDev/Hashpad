...
`scripts/make-doc-icon.ps1` rather than hand-drawn, so it can be reviewed and
...
- [ ] **`.markdown`, `.mdown` and `.mkd` show it too** -- all four registered, or
      none did.
- [ ] **Hashpad.exe itself still shows the "H".** The two icons are meant to be
      a family, not the same picture; if the executable changed, the wrong file
      was wired up.
